...
async def inicializar_tabelas(pool):
    ...
    async with pool.acquire() as conn:
        ...
        # Inserir estoque inicial se não existir
        await conn.execute("""
    ...
    try:
        ...
        for meta in metas:
            ...
            try:
                ...
                erros += 1
        await ctx.send(f"✅ **{atualizadas} painéis atualizados!**\n❌ **{erros} erros**")
    except Exception as e:
        await ctx.send(f"❌ Erro: {e}")
...
async def cmd_verificar_paineis_metas(ctx):
    ...
    try:
        ...
        embed = discord.Embed(title="📊 VERIFICAÇÃO DE PAINÉIS DE METAS", color=0x3498db, timestamp=agora())
        total = len(com_painel) + len(sem_painel) + len(canais_nao_encontrados)
        embed.add_field(name="📊 RESUMO", value=f"**Total de metas:** {total}\n✅ **Com painel:** {len(com_painel)}\n❌ **Sem painel:** {len(sem_painel)}\n⚠️ **Canais não encontrados:** {len(canais_nao_encontrados)}", inline=False)
        if com_painel:
            embed.add_field(name="✅ COM PAINEL", value="\n".join(com_painel[:20]) if com_painel else "Nenhum", inline=False)
        if sem_painel:
            embed.add_field(name="❌ SEM PAINEL", value="\n".join(sem_painel[:20]) if sem_painel else "Nenhum", inline=False)
        if canais_nao_encontrados:
            embed.add_field(name="⚠️ CANAIS NÃO ENCONTRADOS", value="\n".join(canais_nao_encontrados[:10]) if canais_nao_encontrados else "Nenhum", inline=False)
...
async def cmd_historico_metas(ctx, data_inicio: str = None, data_fim: str = None):
    ...
    total_acoes = sum(r.get("dinheiro_acoes") or 0 for r in historico)
    total_polvora = sum(r["polvora"] for r in historico)
    embed = discord.Embed(title="📊 HISTÓRICO DE METAS", description=f"📅 **Período:** {data_inicio} até {data_fim}", color=0x3498db)
    embed.add_field(name="💰 TOTAL DINHEIRO SUJO", value=formatar_dinheiro(total_dinheiro), inline=True)
    embed.add_field(name="🎯 TOTAL DINHEIRO AÇÕES", value=formatar_dinheiro(total_acoes), inline=True)
    embed.add_field(name="💣 TOTAL PÓLVORA", value=f"{fmt_num(total_polvora)} unidades", inline=True)
    lista = ""
...
async def on_ready():
    ...
    # Conectar ao banco de dados (assíncrono)
...
async def iniciar_tarefas_background():
    ...
    try:
        if not atualizar_contagem_alugueis.is_running():
            ...
            logger.info("💰 Loop de contagem de aluguéis iniciado")
    except Exception as e:
        logger.error(f"Erro loop contagem: {e}")
    
    try:
        if not verificar_alugueis_expirados.is_running():
            verificar_alugueis_expirados.start()
            logger.info("💰 Loop de aluguéis expirados iniciado")
    except Exception as e:
        logger.error(f"Erro loop expirados: {e}")
    
    try:
        if not verificar_producoes_ativas.is_running():
            verificar_producoes_ativas.start()
            logger.info("🏭 Loop de produções iniciado")
    except Exception as e:
        logger.error(f"Erro loop produções: {e}")
...
async def carregar_dados_iniciais():
    ...
    try:
        ...
        for r in rows:
            ...
            if data_fim.tzinfo is None:
                data_fim = data_fim.replace(tzinfo=BRASIL)
            alugueis_ativos[r["galpao"]] = {
                "user_id": int(r["user_id"]),
                "fim": data_fim,
                "dias": r["dias"]
            }
        logger.info(f"💰 Aluguéis carregados: {len(alugueis_ativos)}")
    except Exception as e:
        logger.error(f"Erro ao carregar aluguéis: {e}")
    
    # Restaurar produções
    await restaurar_producoes()
    
    # Carregar galpões ativos
    try:
        pool = get_db()
        if pool:
            async with pool.acquire() as conn:
            ...
            logger.info(f"🏭 Galpões ativos restaurados: {len(rows)}")
    except Exception as e:
        logger.error(f"Erro restaurar galpões: {e}")
...
async def registrar_views_persistentes():
    ...
    try:
        views = [
            ...
            SolicitarSalaView(),
            CadastrarLiveView(),
            PainelLivesUnicoView(),
            PainelLivesAdmin(),
            PainelLivesManualView(),
            PolvoraView(),
            ...
            PainelAcoesView(),
            MetaView(0),
            GrupoView("placeholder", "placeholder"),
            ConfirmarExcluirView("placeholder", "placeholder"),
            ControleArmasView(),
            ArmasEntrouView(),
            ArmasSaiuView(),
            ArmasPerdeuView(),
        ...
        logger.info(f"✅ {len(views)} views registradas")
    except Exception as e:
        logger.error(f"Erro ao registrar views: {e}")
