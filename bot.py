...
async def enviar_painel_botao_ausencia():
    ...
    embed = discord.Embed(
        ...
        description=(
            "Clique no botão abaixo para solicitar sua ausência.\n\n"
            "📌 **Como usar:**\n"
...
async def enviar_painel_remover_ausencia():
    ...
    try:
        ...
        embed = discord.Embed(
            ...
            description=(
                "Clique no botão abaixo caso um membro tenha **retornado antes do previsto**.\n\n"
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if dias_ausencia >= 15:
            embed_privado.add_field(
                name="⚠️ Observação",
                value="Por ser uma ausência prolongada (+15 dias), a gerência foi notificada.",
                inline=False
            )
...
class LavagemView(discord.ui.View):
    ...
    async def relatorio(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        # =========================================================
        # EMBED DO RELATÓRIO - ESTILO MÁFIA
        # =========================================================
        embed_resumo = discord.Embed(
        ...
        # Criar lista para enviar em partes (máximo 15 por embed)
        parte = 1
        total_partes = (len(usuarios_ordenados) + 15) // 15

        for i in range(0, len(usuarios_ordenados), 15):
            grupo = usuarios_ordenados[i:i+15]

            embed_lista = discord.Embed(
                title=f"🧼 ── LISTA DE REPASSES ── 🧼",
                description=f"📋 Parte {parte}/{total_partes} • VDR 442",
                color=0x1abc9c,
                timestamp=agora()
            )

            texto = ""
            for idx, (uid, dados_user) in enumerate(grupo, i + 1):
                try:
                    user = await bot.fetch_user(int(uid))
                    if user:
                        # Tenta pegar o apelido no servidor
                ...
                texto += (
                    f"**{idx}.** {nome}\n"
                    f"   📋 {dados_user['quantidade']} lavagem(ns)\n"
                    f"   💰 Sujo: {formatar_dinheiro(dados_user['sujo'])}\n"
                    f"   💵 A repassar: {formatar_dinheiro(dados_user['repassado'])}\n\n"
                )

            embed_lista.add_field(
                name="📋 LISTA DE REPASSES",
                value=texto if texto else "Nenhum registro",
                inline=False
            )

            embed_lista.set_footer(
                text=f"🛡 Vida Rasa 442 • Parte {parte}/{total_partes}",
                icon_url=bot.user.display_avatar.url if bot.user else None
            )

            await canal.send(embed=embed_lista)
            parte += 1
            await asyncio.sleep(0.5)

        # =========================================================
        # MENSAGEM FINAL DE RESUMO RÁPIDO (para facilitar repasses)
        # =========================================================
        texto_resumo = "📋 **RESUMO PARA REPASSES**\n\n"
        for idx, (uid, dados_user) in enumerate(usuarios_ordenados, 1):
            try:
                user = await bot.fetch_user(int(uid))
                if user:
                    member = interaction.guild.get_member(int(uid))
            ...
            texto_resumo += f"{idx}. **{nome}** → 💵 {formatar_dinheiro(dados_user['repassado'])}\n"

        if len(texto_resumo) > 2000:
            # Se for muito grande, enviar em partes
            partes_texto = [texto_resumo[i:i+1900] for i in range(0, len(texto_resumo), 1900)]
        ...
        await interaction.followup.send(
            ...
            f"📨 {total_partes + 2} mensagens enviadas",
            ephemeral=True
        )
...
async def enviar_painel_mensagens():
    ...
    embed = discord.Embed(
        ...
        description=(
            "**Clique no botão abaixo para abrir o menu de mensagens.**\n\n"
    ...
    embed.add_field(
        name="📌 DICA",
        value=(
            "• O passaporte é extraído automaticamente do seu apelido no servidor\n"
...
class MenuMensagensView(discord.ui.View):
    ...
    async def gerar_mensagem(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            ...
            description=(
                "**Selecione o tipo de mensagem que deseja gerar:**\n\n"
                "📌 **Opções disponíveis:**\n"
                "• 📦 Pedido Pronto\n"
                "• ❌ Pedido Cancelado\n"
                "• ✅ Pedido Finalizado\n"
...
# =========================================================
# ==================== PARTE 17: SISTEMA DE CLIPES ========
# =========================================================

# =========================================================
# 1. VARIÁVEIS GLOBAIS DE CLIPES
# =========================================================
fila_clipes = None
clips_postados = set()

# =========================================================
# 2. EVENTO ON_REACTION_ADD - CLIPES
# =========================================================
@bot.event
async def on_reaction_add(reaction, user):
    try:
        global fila_clipes
        if fila_clipes is None:   # ✅ ADICIONE ESTA LINHA
