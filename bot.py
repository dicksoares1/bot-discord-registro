...
async def atualizar_embed_meta(user_id):
    ...
    try:
        ...
        member = guild.get_member(int(user_id))
        if member:
            nome = member.display_name  # Apelido do servidor
        else:
            user = await pegar_usuario(user_id)
...
class RelatorioMetasModal(discord.ui.Modal, title="📊 Relatório de Metas"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # --- EMBED 1: RESUMO GERAL (IGUAL AO FECHAMENTO) ---
        ...
        # --- EMBEDS PARA QUEM PAGOU (IGUAL AO FECHAMENTO) ---
        ...
        # Separar quem pagou (total > 0) e quem não pagou (total == 0)
        pagaram = [r for r in historico_ordenado if (r["dinheiro"] + (r.get("dinheiro_acoes") or 0)) > 0]
        nao_pagaram = [r for r in historico_ordenado if (r["dinheiro"] + (r.get("dinheiro_acoes") or 0)) == 0]
        
        embeds_pagaram = []
        if pagaram:
            for i in range(0, len(pagaram), 10):
                grupo = pagaram[i:i+10]
                embed = discord.Embed(
        ...
        total_embeds = 1 + len(embeds_pagaram) + len(embeds_nao_pagaram)
        await interaction.followup.send(
...
class FecharTodasMetasModal(discord.ui.Modal, title="🔒 Fechar Metas Semanais"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # --- EMBEDS PARA QUEM PAGOU (dividido em blocos de 10) ---
            embeds_pagaram = []
            if relatorio:
            ...
            # --- EMBEDS PARA QUEM NÃO PAGOU (dividido em blocos de 10) ---
            embeds_nao_pagaram = []
            if relatorio:
            ...
            # --- EMBEDS PARA MEMBROS SEM META (dividido em blocos de 10) ---
            embeds_sem_meta = []
            if membros_sem_meta:
            ...
            # Enviar embeds de membros sem meta
            for embed in embeds_sem_meta:
                await canal_resultados.send(embed=embed)
                await asyncio.sleep(0.5)
            
            # Mensagem de confirmação
            total_embeds = 1 + len(embeds_pagaram) + len(embeds_nao_pagaram) + len(embeds_sem_meta)
            await interaction.followup.send(
                f"✅ **Relatório enviado com sucesso!**\n"
