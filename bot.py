...
TOKEN = os.getenv("DISCORD_TOKEN")  # CORRIGIDO
...
class RegistroModal(discord.ui.Modal, title="📋 Registro de Entrada"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send("✅ Registro concluído!", ephemeral=True)
...
class StatusView(discord.ui.View):
    ...
    async def toggle(self, interaction: discord.Interaction, texto: str):
        ...
        base = texto.split("•")[0]
        lista = [l for l in lista if not l.startswith(base)]
        lista.append(texto)
...
class VendaModal(discord.ui.Modal, title="🧮 Registro de Venda"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send("❌ Use apenas números.", ephemeral=True)
            return
        ...
        embed = discord.Embed(title=f"📦 ENCOMENDA {pedido_id}", color=0x1e3a8a)
        ...
        embed.add_field(name="🔫 PT", value=pt)
        embed.add_field(name="🔫 SUB", value=sub)
        ...
        await interaction.followup.send("✅ Venda registrada!", ephemeral=True)
